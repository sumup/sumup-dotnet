...
See `examples/Basic`, `examples/CardReaderCheckout`, `examples/OAuth2`, and `examples/Events` for runnable projects.
...
Create a handler with your endpoint's signing secret and a fallback for unregistered event types:

```csharp
using var client = new SumUpClient();
var events = client.CreateEventsHandler(secret, (notification, _) =>
{
    Console.WriteLine($"Unhandled event {notification.Id}: {notification.Type}");
...
events.OnMemberUpdated(async (notification, cancellationToken) =>
{
    ...
    Console.WriteLine($"Member updated: {member?.Id}");
});

await events.HandleAsync(rawBody, signatureHeader, cancellationToken);
```

Pass the original request bytes and the `X-SumUp-Webhook-Signature` header.
The handler verifies the signature and its five-minute delivery timestamp window before invoking a callback.
Acknowledge delivery after `HandleAsync` succeeds, and use event IDs to deduplicate processing.
`FetchObjectAsync` retrieves the resource's current state; deleted resources may no longer be available.

For manual dispatch, use `client.ParseEventNotification(rawBody, signatureHeader, secret)` and match the returned event type.
See [the ASP.NET Core example](examples/Events) for a complete HTTP endpoint.
...
- `examples/Events` – runs a minimal ASP.NET Core endpoint with signature verification, typed event callbacks, and resource fetching.
...
```sh
export SUMUP_ACCESS_TOKEN="your_api_key"
export SUMUP_EVENT_SECRET="whsec_..."
